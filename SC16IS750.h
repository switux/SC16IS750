...
// MCR register bits
#define	MCR_nDTR					(1 << 0)	// DTR complement
#define	MCR_nRTS					(1 << 1)	// CTS complement
...
#define	IIR_RX_STATUS_ERROR			((1 << 1) | (1 << 2))
#define	IIR_RX_TIMEOUT_ERROR		((1 << 2) | (1 << 3))
#define	IIR_RHR_INTERRUPT			(1 << 2)
#define	IIR_THR_INTERRUPT			(1 << 1)
#define	IIR_MODEM_INTERRUPT			(0x00)
#define	IIR_GPIO_INTERRUPT			((1 << 4) | (1 << 5))
#define	IIR_XOFF_SPECHAR_INTERRUPT	(1 << 4)
#define	IIR_nCTS_nRTS_INTERRUPT		(1 << 5)
...
#define	SPI_PROTOCOL	0

class SC16IS750
{ 
	public:
				SC16IS750( bool, uint8_t );
		int		available( void );
		bool	begin( uint32_t );                               
		bool	flush( void );
		int		peek( void );
		...
		bool	read_GPIO( uint8_t );
		bool	pin_mode( uint8_t, bool );
		bool	write_GPIO( uint8_t, bool );
		bool	set_baudrate( uint32_t );
		bool	set_flow_control_levels( uint8_t, uint8_t );
		bool	set_line( uint8_t, uint8_t, uint8_t );
	...
	private:
		...
		bool	enable_extra_features( void );
		bool 	enable_TCR_TLR( void );
		bool	enable_TX_RX( void );
		bool	FIFO_enable( bool );
		bool    FIFO_reset( bool, bool );
		uint8_t get_IO_state( void );
		int8_t	read_byte( void );
		int8_t	read_register( uint8_t );
		bool	set_IO_direction( uint8_t, bool );
		bool	test( void );
		bool 	toggle_sleep( bool );
